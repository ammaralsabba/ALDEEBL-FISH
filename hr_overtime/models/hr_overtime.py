            ...
            if not holiday_ids:
                ...
                if x==False:
                    ...
                    for attend in attendance_ids:
                        ...
                        overtime = True
                        for policy in contract.policy_id.line_ids:
                            starttime = datetime.strptime(day + " " + '{0:02.0f}:{1:02.0f}'.format(*divmod(policy.starttime * 60, 60)) + ':00', DEFAULT_SERVER_DATETIME_FORMAT)
                            endtime = datetime.strptime(day + " " + '{0:02.0f}:{1:02.0f}'.format(*divmod(policy.endtime * 60, 60)) + ':00', DEFAULT_SERVER_DATETIME_FORMAT)
                            if policy.type=='holiday' and attend.check_in<=endtime and attend.check_out>=starttime:
                                active = policy.active_after
                                overtime = True
                                worked_hours = 0
                ...
                if time:
                    for attendance_id in attendance_ids:
                        ...
                        if attend.check_out:
                            ...
                            check_in = datetime.strptime(attend.check_in,DEFAULT_SERVER_DATETIME_FORMAT)+ timedelta(hours=3)
                            for key,value in sorted(time.items()):
                                ...
                                for policy in contract.policy_id.line_ids:
                                    ...
                                    if policy.type=='restday':
                                        ...
                                        overtime = True
                                        worked_hours = 0
                                        if check_in>=starttime and check_in<start and check_in<=endtime:
                                            worked_hours += (start - check_in).seconds/3600
                                        if now<=endtime and now>=starttime and now>end:
                                            worked_hours += (now - end).seconds/3600
                                        if worked_hours>=active:
                ...
                if working_hours_on_day:
                    ...
                    if leave_type:
                        ...
                        for w in working_hours_on_day:
                            hours +=(w.end_datetime - w.start_datetime).seconds/3600
